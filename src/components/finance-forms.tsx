...
type EditableCategory = Choice & { kind?: string | null };
...
export function CategoryForm({
  ...
  useEffect(() => {
    ...
    const current = categories.find((item) => item.id === selectedId);
    if (!current) return;

    setName(current.name ?? "");
    setKind(
      current.kind === "income" || current.kind === "both"
        ? current.kind
        : "expense",
    );
      ...
      <form onSubmit={submit}>
        <div className={styles.formGrid}>
          ...
          <div className={styles.helperText}>
            {locale === "pt-BR"
              ? "Use este campo para ajustar categorias de receita, despesa ou ambos diretamente na página."
              : "Use this field to adjust income, expense or mixed categories directly on the page."}
          </div>
          ...
          <label className={styles.field}>
            ...
            <select
              ...
              onChange={(event) =>
                setKind(
                  event.target.value as "expense" | "income" | "both",
                )
              }
              value={kind}
            >
        ...
        {kind === "income" || kind === "both" ? (
          <div>
            <p className={styles.helperLabel}>
              {locale === "pt-BR"
                ? "Seções de receita rápidas"
                : "Quick income sections"}
            </p>
